...
export const executeGetContent = async (req, res, next) => {
    try {
        const contentCollectionRef = readLankaFirebaseAppData.readLankaDB.collection("books");

        const contentSnapshot = await contentCollectionRef.get();
        const contents = [];
